    ...
    try:
        ...
        if not wait_for_file_stability(image_path):
            if attempt < MAX_QUEUE_RETRIES:
                ...
                # Requeue the file with increased attempt count
        ...
        with Image.open(image_path) as img:
            ...
            # Determine file extension and save with appropriate settings
            ...
            save_kwargs = {
                ...
                'tiff': {'format': 'TIFF', 'compression': None}  # No compression for maximum quality
            }

            kwargs = save_kwargs.get(file_ext, {})
            
            # Add ICC profile to save kwargs if it exists
            if icc_profile:
